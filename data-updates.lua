...
local tune_autoplace = function(v, is_turret, volume)
    if String.find( v.name, MOD_NAME, 1, true) and v.autoplace then
        if is_turret then
            v.autoplace = AutoplaceUtil.enemy_worm_autoplace(0, FORCE_NAME, volume, 2)
        else
...
if ErmConfig.mapgen_is_mixed() then
    ...
    print('active race:'..active_races)
    if active_races >= 2  then
        ...
        if settings.startup['enemyracemanager-enable-bitters'].value and erm_races['erm_toss'] and active_races > 2 then
            volume.aux_min = 0.59
            volume.aux_max = 1
        elseif erm_races['erm_toss'] and active_races > 2 then
            volume.aux_min = 0.49
            volume.aux_max = 1
        elseif erm_races['erm_toss'] and active_races == 2 then
            volume.water_min = 0
            volume.water_max = 0.51
        end
